#!/usr/bin/env python3
"""
获取美股夜盘榜（get_us_overnight_rank）

功能：获取美股夜盘榜，按指定排序方向返回夜盘交易排行数据，支持多维度筛选。
用法：
  python get_us_overnight_rank.py [--sort-dir 0] [--count 10] [--offset 0] [--config filters.json] [--json]
...
def get_us_overnight_rank(sort_dir=None, count=10, offset=None, config_path=None, output_json=False):
    ctx = None
    try:
        ctx = create_quote_context()
        filter_list = _build_filters(config_path) if config_path else None

        ret, data = ctx.get_us_overnight_rank(
            sort_dir=sort_dir,
            count=count,
            offset=offset,
            filter_list=filter_list,
        )
        check_ret(ret, data, ctx, "获取美股夜盘榜")
        all_count, df = data
        ...
        cols = ["security", "name", "overnight_price", "overnight_change_ratio",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="获取美股夜盘榜 (get_us_overnight_rank)")
    ...
    get_us_overnight_rank(args.sort_dir, args.count, args.offset, args.config, args.output_json)
