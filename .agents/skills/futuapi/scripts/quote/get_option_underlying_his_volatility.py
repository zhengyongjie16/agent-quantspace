#!/usr/bin/env python3
"""
获取历史波动率（get_option_underlying_his_volatility）

功能：获取期权标的历史波动率数据，按交易日返回隐含波动率（IV）与历史波动率（HV）的时间序列。
用法：python get_option_underlying_his_volatility.py US.AAPL --begin 2025-01-01 --end 2025-06-01
...
def get_option_underlying_his_volatility(code, index_option_type=None, begin_time=None, end_time=None, output_json=False):
    ctx = None
    try:
        ...
        while True:
            ...
            check_ret(ret, data, ctx, "获取历史波动率")
        ...
        if output_json:
            ...
            print("=" * 70)
            print(f"期权标的历史波动率 - {code} 共 {len(df)} 条")
            print("=" * 70)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="获取历史波动率 (get_option_underlying_his_volatility)")
    ...
    get_option_underlying_his_volatility(args.code, args.index_option_type, args.begin_time, args.end_time, args.output_json)
