...
功能：获取指定市场的机构列表，支持排序、模糊搜索和自动分页
用法：python get_institution_list.py --market US [--sort-field POSITION_VALUE] [--sort-dir 0] [--count 20] [--name 桥水] [--json]
...
返回字段说明：
- institution_id, institution_name, position_value, position_value_change,
  position_count, position_count_change, disclosure_date, currency
"""
import argparse
import json
import sys
...
SORT_FIELD_MAP = {
    "POSITION_VALUE": futu.InstitutionListSortField.POSITION_VALUE,
    "POSITION_VALUE_CHANGE": futu.InstitutionListSortField.POSITION_VALUE_CHANGE,
    "POSITION_COUNT": futu.InstitutionListSortField.POSITION_COUNT,
    "POSITION_COUNT_CHANGE": futu.InstitutionListSortField.POSITION_COUNT_CHANGE,
}
...
def get_institution_list(market_str, sort_field_str=None, sort_dir_str=None,
                         count=20, name_part=None, output_json=False, no_page=True):
    ...
    try:
        ...
        while True:
            ret, data, next_page, all_count = ctx.get_institution_list(
                market=market, sort_field=sort_field, sort_dir=sort_dir,
                count=count, page=page, name_part=name_part
            )
            check_ret(ret, data, ctx, "获取机构列表")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="获取机构列表")
    ...
    parser.add_argument("--name", dest="name_part", help="机构名模糊搜索")
    ...
    get_institution_list(args.market, sort_field_str=args.sort_field, sort_dir_str=args.sort_dir,
                        count=args.count, name_part=args.name_part, output_json=args.output_json, no_page=args.no_page)
