...
功能：获取指定机构的持股列表，支持变动类型筛选、排序、关键词搜索和自动分页
用法：python get_institution_holding_list.py --market US --institution-id 123 [--change-type INCREASE] [--sort-field HOLDING_VALUE] [--sort-dir 0] [--count 20] [--keyword TSLA] [--json]
...
- --keyword: 搜索关键词（股票名/代码）

返回字段说明：
- security, name, industry_name, holding_value, holding_pct, last_holding_pct,
  change_shares, portfolio_pct, change_pct, holding_date, source, currency
"""
import argparse
import json
import sys
...
SORT_FIELD_MAP = {
    "HOLDING_VALUE": futu.InstitutionHoldingListSortField.HOLDING_VALUE,
    "HOLDING_PCT": futu.InstitutionHoldingListSortField.HOLDING_PCT,
    "LAST_HOLDING_PCT": futu.InstitutionHoldingListSortField.LAST_HOLDING_PCT,
    "CHANGE_SHARES": futu.InstitutionHoldingListSortField.CHANGE_SHARES,
    "CHANGE_PCT": futu.InstitutionHoldingListSortField.CHANGE_PCT,
    "PORTFOLIO_PCT": futu.InstitutionHoldingListSortField.PORTFOLIO_PCT,
    "INDUSTRY": futu.InstitutionHoldingListSortField.INDUSTRY,
    "HOLDING_DATE": futu.InstitutionHoldingListSortField.HOLDING_DATE,
}
...
def get_institution_holding_list(market_str, institution_id, change_type_str=None,
                                 sort_field_str=None, sort_dir_str=None, count=20,
                                 keyword=None, output_json=False, no_page=True):
    ...
    try:
        ...
        while True:
            ...
            check_ret(ret, data, ctx, "获取机构持股列表")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="获取机构持股列表")
    ...
    parser.add_argument("--change-type", choices=list(CHANGE_TYPE_MAP.keys()), help="变动类型筛选")
    ...
    parser.add_argument("--keyword", help="搜索关键词（股票名/代码）")
    ...
    get_institution_holding_list(args.market, args.institution_id,
                                change_type_str=args.change_type, sort_field_str=args.sort_field,
                                sort_dir_str=args.sort_dir, count=args.count,
                                keyword=args.keyword, output_json=args.output_json, no_page=args.no_page)
