#!/usr/bin/env python3
"""
获取末日期权标的列表（get_option_zero_dte_screener）

功能：获取末日期权标的筛选列表，返回当日到期（0DTE）期权对应的标的股票信息。
用法：
  python get_option_zero_dte_screener.py --market US_SECURITY --sort-type VOLUME --count 20
  python get_option_zero_dte_screener.py --market US_SECURITY --config filters.json

JSON 配置示例（filters.json）：
{
  "filters": [
    {"indicator_type": "IV", "interval_min": 30.0},
    {"indicator_type": "HAS_EARNINGS_THIS_WEEK", "value_list": [1]},
...
def _resolve_sort_type(sort_str):
    if not sort_str:
        return None
    from futu import ZeroDteSortType
    key = str(sort_str).strip().upper()
    if hasattr(ZeroDteSortType, key):
        return getattr(ZeroDteSortType, key)
    raise ValueError(f"无效的 ZeroDteSortType: {sort_str}")


def _build_filters(spec):
    if not spec or not spec.get("filters"):
        return None
    from futu import ZeroDteIndicatorType
    from futu.quote.quote_option_event_info import ZeroDteFilter
    ...
    for f in spec["filters"]:
        ...
        indicator_type = getattr(ZeroDteIndicatorType, str(indicator_str).upper())
        ...
        filters.append(ZeroDteFilter(**kwargs))
    return filters


def get_option_zero_dte_screener(market, sort_type=None, is_asc=None, count=None,
    ...
    try:
        ...
        while True:
            if page:
                kwargs["page"] = page
            ret, data = ctx.get_option_zero_dte_screener(**kwargs)
            check_ret(ret, data, ctx, "获取末日期权标的列表")
        ...
        if not all_rows:
            if output_json:
                print(json.dumps({"market": market, "data": []}))
            else:
                print("无数据")
            return
        ...
        if output_json:
            ...
            print("=" * 70)
            print(f"末日期权标的列表 - market={market} 共 {len(df)} 条")
            print("=" * 70)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="获取末日期权标的列表 (get_option_zero_dte_screener)")
    parser.add_argument("--market", required=True, help="期权市场: US_SECURITY, US_INDEX, HK_SECURITY, HK_INDEX")
    parser.add_argument("--sort-type", help="排序类型: VOLUME, IV, CHANGE_RATIO, OPEN_INTEREST, MARKET_CAP")
    ...
    get_option_zero_dte_screener(args.market, args.sort_type, args.is_asc if args.is_asc else None,
